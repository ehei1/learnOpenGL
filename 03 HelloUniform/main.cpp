...
int main()
{
	...
	if (std::ifstream file(R"(./03_vertex.glsl)"); !file.is_open()) {
	...
	{
		std::ifstream fstream(R"(./03_vertex.glsl)");
	...
	if (std::ifstream file(R"(./03_fragment.glsl)"); !file.is_open()) {
		throw std::invalid_argument("file is not found");
	}
	...
	{
		std::ifstream fstream(R"(./03_fragment.glsl)");
		std::stringstream sstream;
		sstream << fstream.rdbuf();
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// get the uniform for the vertex color
